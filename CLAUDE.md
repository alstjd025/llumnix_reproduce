...
| **남은 방향** | **EXP-50(후보 C, 수정 프로파일)이 돌고 있다.** 계수기가 페이스 게이트를 거절의 90.8%로 지목했고 C가 그것을 바꾸는 유일한 제안이다. 판정 규칙 다섯 개 기작성 — 정적 +4.2점 이상, preemption 3,000 미만, 게이트 점유율이 실제로 내려갈 것, dr attainment가 오를 것. 그다음 ① 후보 B(SHED에 KV 부담) ② knee 확정 |
| **후보 현황** | **H0**(길이 프로파일 수정) **채택 — 최대 이득**(정적 +20.2, 동적 +10.4). **H2 기각**(투영은 고쳤으나 offered −2.4). **A** 정적 채택·동적 무효. **C** 정적 최대(+23.7)·동적 기각이었으나 **그 기각이 틀린 입력 위에서 났고 게이트가 거절의 90.8%이므로 EXP-50에서 재측정 중**. **E** null. **D·F·G·H** 폐기/철회. **B** 미착수 |

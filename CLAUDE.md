...
| implementation.md **§60~§61** | **여기부터 읽는다. 지금 상태의 정본.** §60 EXP-56 클래스 선호 ablation — **H1 확인**(45 req/s에서 +12.2/+6.9/+3.4, 평균 +7.5, 문턱 5.0; 55에서는 평균 +4.8로 문턱 아래이나 여섯 쌍 전부 같은 부호), **H3 확인**(산포 8.5 → 0.6). **기전이 작동했다는 직접 증거**: 창별 클래스 집중도가 51.5/61.2/63.3%인데 끄면 27.4/29.4/30.5%(균등 25.0). **H2는 사전 등록이 틀렸다** — 게이트 신호와 균질성 신호를 상호배타로 적었는데 **둘 다 나온다. 같은 원인(분리)의 두 면이기 때문**이고, 기여도를 가르려면 arm이 하나 더 필요하다. **§60.4: 45 req/s의 "두 상태"는 하나의 연속 축이었다** — 점수와 "chat 없는 인스턴스가 존재한 샘플 비율"의 상관이 **+0.953**(45 안에서만; 풀링하면 +0.444). ⚠ **§61이 차단 항목**: 노드가 **DiskPressure**라 `fsnoaff/full`이 죽었고 **EXP-57을 러너 Job 뜨기 전에 중단시켰다**(오염 없음). `results/` 1.5 TB 중 **1.42 TB가 같은 데이터 두 벌** |
...
- **노드가 디스크로 막히면 실험은 정책이 아니라 고장을 잰다 (2026-08-05, §61).**
  ...
  원인은 누적이다 — `results/`가 1.5 TB인데 **약 1.42 TB가 토큰별 타이밍 이벤트이고 두 벌
  있다**: `run_experiment.py`의 `_merge_load_shards`가 워커별 shard를 표준 파일로 합친 뒤
  **shard를 안 지운다**(합친 파일 7.80 GB 대 shard 합계 7.80 GB, 비율 1.0000).
  한 시간 run 하나가 약 16 GB를 쓴다. 반면 거의 모든 분석이 읽는 `metrics.csv`는 832개
  전부 합쳐 **8.8 GB**다.
  → **삭제는 사용자 지시가 있어야 한다.** 선택지와 회수량은 implementation.md §61.3.

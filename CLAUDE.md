...
| **남은 방향** | **H2(§48.5)가 다음** — `inflow`/`outflow` 대신 **인스턴스별 KV 점유량의 관측 변화율**을 EWMA로 유지해 horizon만큼 외삽한다. 오프라인 채점에서 편향이 없고 MAE가 배포본의 1/3.5. 설계·판정 규칙은 **EXP-49 파일에 실행 전에 적어 뒀다**(rule 0이 기전 확인이다). 그다음 ① H2가 되면 **후보 C를 다시 얹는다**(정적 +23.7이었고 기각 사유가 오직 preemption) ② 후보 B(SHED에 KV 부담, 사용자 제안) ③ knee 확정(36·40·45 req/s, 반복 4회+) |
| **후보 현황** | **A** 정적 채택(60에서 +13.1)·동적 무효(산포 6.4). **C** 정적 채택(+23.7, 최대)·동적 기각(preemption 6,583). **E** null. **D·F·G** 전제 반증으로 폐기. **H** §48.1에서 철회(`expectedToks`가 프로파일에서 오는데 그 값이 282라 dr을 3.8%만 조인다). **H0**(프로파일 수정) 적용됨, EXP-48 진행 중. **H2** 구현·스테이징 완료(`c3c6a3518092ea80eb7bb4dc1725ae5b`), EXP-49. **B** 미착수 |

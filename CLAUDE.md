...
| [ms_dev/notes/vllm-router-baseline.md](ms_dev/notes/vllm-router-baseline.md) | **vLLM production stack router를 기준선으로 세우는 계획 (정본, 2026-08-10).** ⚠ **"vLLM router"라는 이름의 물건이 둘이다** — vllm-project/production-stack(Python, Helm 기본값 `roundrobin`)과 PyPI `vllm-router`(Rust, 기본값 `cache_aware`). **이식 대상은 앞의 것이고 `roundrobin`이다.** §2가 소스에서 확인한 것(엔진 통계를 받아 놓고 안 쓴다), §4가 **거절 0%인 arm이 반드시 통과해야 하는 유효성 검사**, §5가 실행 전에 적은 판정 규칙 |

...
| **남은 방향** | **EXP-49(H2)가 돌고 있다** — 투영을 인스턴스별 KV 점유량의 관측 변화율로. rule 0이 기전 확인(배포된 투영의 편향이 ±20,000 이내, 과소예측 40~60%)이고 rule 1이 preemption 500 미만. 그다음 ① **후보 C 재측정**(수정 프로파일 위에서) ② 후보 B(SHED에 KV 부담) ③ knee 확정 |
| **후보 현황** | **H0**(길이 프로파일 수정) **채택 — 최대 이득**(정적 +20.2, 동적 +10.4). **A** 정적 채택(+13.1)·동적 무효. **C** 정적 최대(+23.7)·동적 기각(preemption 6,583) — **그 기각이 틀린 입력 위에서 났으므로 H2 뒤에 다시 잰다.** **E** null. **D·F·G·H** 폐기/철회. **H2** 구현 완료(`c3c6a3518092ea80eb7bb4dc1725ae5b`), **EXP-49 진행 중**. **B** 미착수 |

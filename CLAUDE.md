...
| implementation.md **§62** | **여기부터 읽는다. 지금 상태의 정본 (2026-08-06).** motivation 재구성 + 지표 교체 + **코드 변경 둘** + **실험 둘이 도는 중**. ① **EXP-58**(진행 중, 01:53 KST 시작, 약 5시간): 클래스 선호를 켜기/끄기가 아니라 **0~1의 세기 `w`**로 만들었다(`score = w·share + (1−w)·room`, `--fluidserve-affinity-weight`, 배포 `cf74fe98`). **w=1이 지금까지의 동작과 같고 w=0이 `affinity=false`와 같다** — 끝점이 이미 세 번씩 측정된 조건이라 그 자체가 검증이다. ② **EXP-59**(연쇄 대기 중, 약 3시간): `--fluidserve-class-pin`으로 **클래스를 인스턴스에 고정하는 모드**(staging `c2d970ca`, 미배포). arm 둘 — `fspin_poly`(chat 1/dr 2/swe 1, PolyServe가 고른 배분)와 `fspin_demand`(chat 2/dr 1/swe 1, 출력 토큰 비례). **둘을 두는 이유는 "배정이 고정된 것"과 "고른 배정이 틀린 것"이 지금 안 갈려 있어서**다. §62.6에 **검토에서 나와 즉시 고친 다섯 개**(§2.1 굵은 문장이 자기 표와 어긋남, §7.1이 EXP-53 PolyServe 값을 씀, +2.1/−1.8 불일치, 경고를 어긴 제안 문장, **EXP-56 §6.4의 설명이 틀림 — 그 항은 두 arm 모두 이미 꺼져 있었고 애초에 그 값에 작용할 수 없다**), §62.7에 **계측 정정**(chat 없는 인스턴스-시각의 정의) |

...
**2026-08-21 재측정으로 값이 바뀌었다. 결론의 방향은 같고 크기가 다르다.**
여기 있던 낡은 값 gate 92~96%, memory 0%(EXP-67b)는 인용하면 안 된다 — 2026-08-08 워크로드
수정 이전의 것이다. 고정된 다섯 arm 세트의 FluidServe 16 run(여덟 도착률 × 2반복)에서
같은 계수기를 다시 읽은 값은 이렇다(정본
...
| 도착률 | 게이트 | incumbents | **메모리** |
|---|---|---|---|
| 10~20 req/s | 84~98% | 1~3% | **0~13%** |
| 25 req/s | 82% | 17% | 1% |
| 35~70 req/s | **71~82%** | 7~11% | **17~20%** |

**게이트가 여전히 지배적이므로 이 절의 주장은 유지된다.** 바뀐 것 둘: 우리가 실제로 다투는
25 req/s 위에서 게이트가 71~82%이고(낡은 값 92~96%는 인용하면 안 된다),
**메모리가 0%가 아니라 17~20%다.**
"KV는 제약이 아니다"라고 쓰면 안 된다 — 그것은 함대에 여유가 있는 저부하에서만 참이다.
EXP-68의 엔진 계층이 같은 곳을 가리킨다(§4.2).

...
이고, 클라이언트의 `last_request_id`가 이미 그 본문 `id`를 저장한다. **`cmpl-` prefix만 떼면
...
| 3 | **요청을 가르는 신호** | **클래스**(`share`). **prefix는 안 본다**(§9.8) | **prefix**(블록 해시 일치율) | **코드 확인** | 클래스당 유효 인스턴스 수, 엔진별 prefix hit rate |
...
| 어디 | 어떻게 | 무엇을 아는가 |
|---|---|---|
| KV 용량 모델 | `kvPhysical / kvLogical` 비율을 **측정**해서 쓴다 | prefix 공유로 물리 KV가 논리보다 작다는 것 (`fluidserve.go:1032`) |
...
가능:  prefill 비용 = t_pre(프롬프트 토큰 × (1 − 그 인스턴스의 prefix 일치율))
```

⚠ **그런데 넣으면 우리 주장 하나가 흔들린다.** "분리를 구성하지 않고 결과로 얻는다"의 기전은
클래스 선호(`share`)인데, **prefix 선호를 넣으면 분리를 만드는 신호가 둘이 되어 어느 쪽이 만든
분리인지 갈리지 않는다.** 그리고 우리 세 클래스는 프롬프트 구조가 크게 달라 prefix만으로도
갈릴 가능성이 있다.

→ **순서**: ① EXP-66의 H3이 먼저 답한다(prefix 선호만으로 클래스가 갈리는가). 갈리면 prefix
항을 넣는 것은 주장을 흐리는 일이다. ② 안 갈리면 `share`와 직교하는 항이므로 별개로 넣고
ablation으로 기여를 가를 수 있다. ③ **어느 쪽이든 "라우팅에는 prefix를 안 쓰고 용량 모델에는
측정으로 흡수한다"를 논문에 적는다** — 지금 어디에도 없고 심사에서 반드시 나온다.

---

## 9.9 두 반복 결과 (rep 1 2026-08-07 22:56 KST, rep 2 2026-08-08 02:29 KST 완료)

**요청 단위 offered 달성률과 token goodput. 괄호는 두 반복의 폭.** FluidServe는 EXP-53의
같은 arm 두 반복이다(다른 세션 — 아래 caveat).
...
### rep 1만 볼 때와 이야기가 다르다 — 네 가지

1. **45 req/s 아래에서는 우리가 이긴다.** 세 rate 전부 100.0 대 94.8~98.5이고 goodput도
   더 높다. **경쟁이 없을 때 llm-d가 1.5~5.2점을 잃는다.** rep 1은 15 req/s에서 100.0이었고,
   그래서 "45부터 llm-d가 앞선다"로 읽었는데 **실제로는 45가 교차점이다.**
2. **45는 무승부다** — 90.2 대 90.5로 llm-d의 반복 폭 1.8 안이다.
3. **50~60에서 llm-d가 9.7~13.0점 앞선다.** 여기가 확립된 격차다.
4. **70에서는 확립되지 않았다.** 차이가 +6.0인데 **llm-d의 두 반복이 63.7과 52.0으로 11.7점
   벌어진다.** FluidServe도 60 req/s에서 66.7과 55.0으로 똑같이 11.7점 벌어진다.
   **두 시스템 다 그 근처에서 불안정하고, 한 반복으로 그 구간을 인용하면 안 된다.**

**철회**: rep 1만으로 적은 "55에서 +17.9, 70에서 +14.2"(등가중)는 `retracted.tsv`에 넣었다.
등가중 두 반복 값은 55에서 82.2(폭 4.1), 70에서 66.0(폭 0.2)이다.

### 격차는 거의 전부 swe이고, 우리는 3배 더 거절한다

55 req/s 클래스별 offered(두 반복 평균):
...
**swe는 두 시스템이 다른 예산을 받는 유일한 클래스다** — 우리는 m1 + `25:e2e:30000`,
llm-d는 m1f의 (2,500, 52). 채점은 둘 다 30초 E2E로 같지만 정책이 받는 입력이 다르다.
정책 차이인지 예산 해석 차이인지 이 표로는 못 가른다.
→ 가르려면 **FluidServe를 m1f로 한 번 돌리는 arm**이 필요하다.

**그림**: `results/aggregate_analysis/exp66/`(두 반복). rep 1만의 세트는 `exp66r1/`에 남아
있다. 정본은 각 디렉토리의 `README.md`이고, 다시 만드는 명령은
`bash analysis_scripts/redraw_static_sweep_llmd.sh results/aggregate_analysis/exp66 'results/*exp66r*_llmdslo_m1f_rpm_*'`.

### ⚠ 아직 인용하면 안 되는 이유 둘 (넷이었는데 둘 해제)

1. ~~반복 1회~~ → **rep 2로 해제.** 다만 **반복 폭이 rate마다 다르다**: 50~60에서는 0.2~0.6인데
   70에서는 11.7이다. **읽으려는 차이가 그 rate의 폭보다 큰지 매번 확인한다.**
2. ~~예열 주행~~ → **2026-08-08에 재고 해제(§9.11).**
3. **세션이 다르다** — FluidServe는 08-02, llm-d는 08-07~08.
4. **m1f 대 m1** — swe에서만, 그리고 격차가 swe에 있다.

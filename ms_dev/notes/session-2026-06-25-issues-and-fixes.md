...
| ~~M9~~ | ~~migration ON이면 출력 손상~~ → **M10에서 해결됨** | (중간 오결론) backend `"kvt+migration"`으로 켰더니 출력 손상이라 "migration=출력 손상"으로 잘못 결론냈었음. 진짜 원인·해결은 M10. |
| **M10** | **✅ migration ON 출력 손상의 진짜 원인 = 잘못된 backend** | **원인**: backend `"kvt+migration"`은 Blade-KVT의 **PD-분리(prefill/decode) 백엔드**를 켠다. 이건 **모든 요청을 "원격에서 KV를 load해야 하는 consumer"로 취급**(`ali_llumnix_disagg` 기본 true). neutral 모드에선 게이트웨이가 `do_local_prefill`을 안 set하므로 → 엉뚱/없는 KV를 load → **엔진1개는 abort("no p"), 엔진N개는 peer의 KV를 load해 garbage**. 코드: `kvtbackend.py` `get_operations`(disagg면 원격 load), `hybrid_modules.py:284`. **해결**: backend를 **`"migration"` 단독**으로(= `MigrationBackend`만, PD-load 안 함). 요청은 로컬 prefill, 실제 마이그레이션 때만 KV 전송. **검증**: backend `migration` + `--disable-custom-all-reduce` → 4엔진 일관 정상("Paris"/"Tokyo", finish=length), 부하 후에도 정상, MigrationFrontend ×4. 커밋 `34c434e`. **공식 레시피가 전부 PD(prefill/decode 분리)인 이유가 이것 — HybridConnector는 PD 전용이고, neutral 단독 migration은 `backend:"migration"`이어야 한다.** |

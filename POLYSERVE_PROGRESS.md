...
### 2026-07-25 — EXP-21 스모크: P2·P3가 실제 런에서 작동함을 확증

600 rpm × 2분, 양쪽 arm. attainment는 둘 다 100%(이 rate가 너무 쉬워 변별력 없음 — 스모크의
목적은 파이프라인 검증이었고 그건 완전히 달성). **결정적 결과는 라우팅 집중도**:
...
swe의 0.333은 **4대 중 2대에 갇힌 클래스의 이론값과 정확히 일치**한다. chat/dr은 1대씩
(0.95대인 이유는 첫 재분할이 적용되기 전 ~30초분 요청이 섞여서). 즉 재분할이
**swe 2 / chat 1 / dr 1** 로 수렴했고 tier 격리가 전 구간 작동했다 —
설계 문서가 예측한 배분과 같고, P3 단독 테스트 결과와도 같다.

admission 거부는 148건 전부 이유가 **"steady state"**(`iterMax > tpotSlo`)였다. 예상했던
리스크 2번(prefill 간섭항이 iterMax를 부풀림)이 실제로 지배적이라는 첫 신호다.

**함정 기록 3**: 스케줄러가 `-v 4`에서 초당 ~287줄을 뱉어 컨테이너 로그가 회전한다.
25분치를 요청해도 실제로는 **66초분만** 남는다. 그래서 런 초반의 `PolyServe repartition`
줄은 사라진다 — 메커니즘 확인은 로그가 아니라 내구성 있는 산출물(라우팅 집중도)로 할 것.

...
func TestThePrefillFractionIsMeasuredFromTheWorkTheEngineDid(t *testing.T) {
	...
	for i := 0; i < 2000; i++ {
		m.notePrefill(dec+504.0/100, dec, 100, 8192, 81920, false)
	}
	...
	for i := 0; i < 5000; i++ {
		m.notePrefill(dec, dec, 100, 8192, 81920, false)
	}
	...
	m.notePrefill(dec+100, dec, 100, 8192, 0, false)
...
func TestProgressComesFromTheStepCounter(t *testing.T) {
	...
	r.onDispatch("e0", "a", 50, 16384, 8192, 1000, now, 0, 0, 0)
...
func TestReconcileTrimsToTheEngineCount(t *testing.T) {
	...
	r.onDispatch("e0", "a", 50, 1000, 8192, 1000, now, 0, 0, 0)
	r.onDispatch("e0", "b", 50, 1000, 8192, 1100, now, 0, 0, 0)
	r.onDispatch("e0", "c", 50, 1000, 8192, 1200, now, 0, 0, 0)
...
func TestRecordsDropWhenTheEngineRestarts(t *testing.T) {
	...
	now := int64(1_000_000)
	r.noteArrival("a", 0, now)
	r.onDispatch("e0", "a", 50, 1000, 8192, 5000, now, 0, 0, 0)
...
func TestRequestsPastEveryObservedLengthAreRetired(t *testing.T) {
	...
	now := int64(1_000_000)
	r.noteArrival("a", 0, now)
	r.onDispatch("e0", "a", 50, 100, 8192, 1000, now, 0, 0, 0)
...
func TestEndToEndBudgetCountsQueueingTime(t *testing.T) {
	...
	r.onDispatch("e0", "s", 25, 20000, 8192, 1000, now+5000, 0, 0, 0)
	...
	r2.onDispatch("e0", "s", 25, 20000, 8192, 1000, now+5000, 0, 0, 0)
